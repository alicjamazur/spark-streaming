...
object LongestTweetLength {
  ...
  def main(args: Array[String]) {
    ...
    lengths.foreachRDD((rdd, time) => {
      ...
      var count = rdd.count()
      if (count > 0) {
        totalTweets.addAndGet(count)
        
        totalChars.addAndGet(rdd.reduce((x,y) => x + y))

//        for (tweet <- rdd)
//          {println(tweet.reduce(x,y) =>
//              {if (x>y) {x} else y}
//          }
//        )
        ...
        println("Total tweets: " + totalTweets.get() + 
            ...
            " Average: " + totalChars.get() / totalTweets.get() +
              " Longest tweet: " + longestTweet.get())
      }
    })
    
    // Set a checkpoint directory, and kick it all off
    // I could watch this all day!
    ssc.checkpoint("/Users/alicjamazur/Desktop/all-projects/spark-streaming-course/tweets-streaming/checkpoint2")
    ssc.start()
